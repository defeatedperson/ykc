...
if (isset($_POST['action']) && $_POST['action'] === 'verify_account') {
    ...
    // 获取并校验输入
    $username = isset($_POST['username']) ? validate_common_input($_POST['username']) : false;
    $password = isset($_POST['password']) ? validate_password($_POST['password']) : false;
    $account_temp_jwt = isset($_POST['account_jwt']) ? $_POST['account_jwt'] : '';
    // 校验临时token（必须为account场景）
    $jwt_check = validate_temp_jwt($ip, $account_temp_jwt);
    $is_account_scene = ($jwt_check['valid'] && $jwt_check['scene'] === 'account');
    // 输入内容安全性判断
    if ($username === false || $password === false || !$is_account_scene) {
        header('Content-Type: application/json');
        echo json_encode(['status' => 'account_failed']);
        exit;
    }
    // 账号密码验证
    $verify = verify_account($username, $password);
